...
export default class PlayAbnormalScene extends Phaser.Scene {
  constructor() {
    super("PlayAbnormalScene");
  }

  preload() {
    // Assets gambar
    this.load.image("bg deep", "assets/Abnormal/bg deep.png");
    this.load.image("board batu", "assets/Abnormal/board batu.png");
    ...
    this.load.image("card8", "assets/wizard.png");
    this.load.image("card9", "assets/mask.png");
    this.load.image("card10", "assets/eyes.png");
    this.load.image("card11", "assets/ghost.png");
    this.load.image("card12", "assets/kuburan.png");
    ...
    this.load.image("meja deep", "assets/Abnormal/meja deep.png");
  ...
  create() {
    ...
    this.add.image(centerX, centerY, "bg deep").setDisplaySize(width, height);
    this.add.image(centerX, height - 410,"meja deep").setScale(1.2).setDepth(0);
    this.add.image(centerX, centerY - 90, "board batu").setScale(1, 0.85).setDepth(1);
  ...
  createCards(centerX, centerY) {
      ...
      const totalWidth = (cols - 1) * cardSpacingX;
      const totalHeight = (rows - 1) * cardSpacingY;
  
      const startX = centerX - totalWidth / 2;
      const startY = centerY - totalHeight / 1.47;
  ...
  flipCard(card) {
    ...
    this.tweens.add({
      ...
      duration: 150,
      onComplete: () => {
        card.setTexture(card.actualTexture).setScale(0.33);
        this.tweens.add({
          targets: card,
          scaleX: 0.36,
          duration: 150,
  ...
  flipToBack(card) {
    this.tweens.add({
      ...
      duration: 150,
      onComplete: () => {
        card.setTexture("cardBack").setScale(0.72);
        this.tweens.add({ 
          targets: card, 
          scaleX: 0.8, 
          duration: 150 
    ...
    if (card1.actualTexture === card2.actualTexture) {
      ...
      this.time.delayedCall(800, () => {
        this.tweens.add({
          ...
          duration: 150,
          onComplete: () => {
            card1.setTexture("cardBack").setScale(0.68);
            card2.setTexture("cardBack").setScale(0.68);
