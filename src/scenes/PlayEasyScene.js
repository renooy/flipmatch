...
export default class PlayEasyScene extends Phaser.Scene {
  constructor() {
    super("PlayEasyScene");
  }
  ...
  preload() {
    // Assets gambar
    this.load.image("bg hutan", "assets/easy/bg hutan.png");
    this.load.image("board", "assets/easy/board.png");
    ...
    this.load.image("meja", "assets/easy/mejabwh.png");
  ...
  create() {
    ...
    this.add.image(centerX, centerY, "bg hutan").setDisplaySize(width, height);
    this.add.image(centerX, height - 60, "meja").setScale(1.2).setDepth(0);
    this.add.image(centerX, centerY - 60, "board").setScale(0.9).setDepth(1);
  ...
  createCards(centerX, centerY) {
    ...
    const startY = centerY - totalHeight / 1.3;

    const cardTextures = ["card1", "card2", "card3", "card1", "card2", "card3"];
  ...
  createUI() {
    ...
    this.papanNyawa = this.add.image(120, height - 200, "papan_nyawa").setScale(0.8).setDepth(1);
    const nyawaStartX = 35, nyawaY = height - 200, spacing = 80;
